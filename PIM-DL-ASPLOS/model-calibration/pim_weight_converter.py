...
def read_weight(args):
    layer_tensors = {}
    layer_num = 0
    # Force CPU loading so conversion works on machines without NVIDIA/CUDA.
    with safe_open(args.weight_input_file, framework="pt", device="cpu") as f:
        for k in f.keys():
            split_k = k.split('.')
    ...
    for i in range(layer_num):
        ...
        for op_name in weight_tensors.keys():
            ...
            if op_name not in centroid_tensors:
                missing_centroid_ops.append(op_name)
                continue
    ...
    if len(missing_centroid_ops) > 0:
        sample = ', '.join(sorted(set(missing_centroid_ops))[:8])
        raise ValueError(
            "Missing '.centroids.weight' for some transformer weights. "
            "This converter expects a LUT-NN calibrated checkpoint, not a vanilla HuggingFace checkpoint. "
            f"Sample missing ops: {sample}"
        )
