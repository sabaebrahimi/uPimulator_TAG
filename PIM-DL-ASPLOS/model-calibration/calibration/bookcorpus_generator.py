...
try:
    ...
    # Newer huggingface_hub versions removed Repository from top-level exports.
    # This script does not require Hub repository operations for local runs.
...
def main():
    ...
    if args.load_tokenized_datasets is None:
        if args.dataset_name is not None:
            ...
            raw_datasets = None
            dataset_candidates = [(dataset_name, dataset_config_name)]
            if dataset_config_name is not None:
                dataset_candidates.append((dataset_name, None))
            if dataset_name in {"bookcorpus", "bookcorpusopen"}:
                dataset_candidates.extend(
                    [
                        ("bookcorpus/bookcorpus", None),
                        ("SamuelYang/bookcorpus", None),
                    ]
                )
            ...
            for candidate_name, candidate_config in dataset_candidates:
                try:
                    logger.info(f"Trying dataset source: name={candidate_name}, config={candidate_config}")
                    raw_datasets = load_dataset(candidate_name, candidate_config)
                    dataset_name = candidate_name
                    dataset_config_name = candidate_config
                    break
                except Exception as exc:
                    last_exc = exc
                    logger.warning(
                        f"Failed dataset source name={candidate_name}, config={candidate_config}: {exc}"
                    )

            if raw_datasets is None:
                raise RuntimeError(
                    "Unable to load dataset from all known BookCorpus sources. "
                    "Please provide --train_file/--validation_file for local text data."
                ) from last_exc
