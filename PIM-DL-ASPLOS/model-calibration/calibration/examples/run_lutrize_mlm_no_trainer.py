...
try:
    ...
    # Newer huggingface_hub versions removed Repository from the top-level API.
    # This script only needs it when --push_to_hub is enabled.
    ...
    if accelerator.is_main_process:
        if args.push_to_hub:
            if Repository is None:
                raise ImportError(
                    "huggingface_hub.Repository is not available in your installed version. "
                    "Either install an older huggingface_hub version that provides Repository "
                    "or run without --push_to_hub."
                )
